...
{chr(10).join([f'  nlohmann_json_t.{name} = {f.get("from_prefix", "")}nlohmann_json_j.value("{name}", {f.get("to_prefix", "")}nlohmann_json_default_obj.{name}){f.get("from_suffix", "")};' for name, f in cpp_fields.items()])}
}}
"""
        cpp_class_defs[
            name
...
{field_decls}
 ...
 public:
  ...
  {name}(const {name}& other): {", ".join([f'{name}({f.get("from_prefix", "")}{f.get("to_prefix", "")}other.{name}{f.get("from_suffix", "")})' for name, f in cpp_fields.items()])} {{}}
...
"""
    ...
    if (nlohmann_json_t.tag_ == Tag::{name.upper()}) {{
      ...
      return;
                ...
                for idx, (name, ty) in enumerate(cpp_fields.items())
            ]
        )
    ...
    if (nlohmann_json_j.contains("{name}")) {{
      nlohmann_json_t.variant_.emplace<{idx + 1}>(nlohmann_json_j.at("{name}"));
